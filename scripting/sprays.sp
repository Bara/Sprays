...
void ListSprays(int client, const char[] category)
{
    ...
    for (int i = 1; i < g_iCount; ++i)
    {
        ...
        else if (g_iSprays[i][bValve] && (g_cValveFlag.BoolValue && HasClientFlags(client, g_cVIPFlag)))
        {
            menu.AddItem(sItem, g_iSprays[i][sSpray]);
        }
    }
...
public Action Command_MySpray(int client, int args)
{
    ...
    if (g_iSprays[g_iSpray[client]][bValve] && !(g_cValveFlag.BoolValue && HasClientFlags(client, g_cVIPFlag)))
    {
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse)
{
    ...
    if (buttons & IN_USE)
    {
        ...
        if (g_cValveFlag.BoolValue && HasClientFlags(client, g_cVIPFlag))
        {
        ...
        if(g_iSpray[client] == 0)
        {
            ...
            if (g_iSprays[rand][bValve] && !(g_cValveFlag.BoolValue && HasClientFlags(client, g_cVIPFlag)))
            {
                g_iSpray[client] = 0;
                SetClientCookie(client, g_hCookie, "0");
